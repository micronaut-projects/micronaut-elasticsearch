...
Once you have the above configuration in place then you can inject the `co.elastic.clients.elasticsearch.ElasticsearchClient` the `org.elasticsearch.client.RestClient` or the deprecated `org.elasticsearch.client.RestHighLevelClient` bean. The following is the simplest way to get Elasticsearch information using the ElasticsearchClient:
